...
The RDE Leal model was chosen as the RDE to investigate with the DWD due to its simplicity and superior predictive ability.
...
\small
\captionsetup{justification=centering}
	\caption{Best fit zero dose average intensity values ($I_{ND} = I(D=0,h)$ (arb. units)) for each resolution bin. Note that $1/h_{mid} = d_{mid}$ where $d_{mid}$ is the mid-point resolution in each resolution bin}
	\centering
