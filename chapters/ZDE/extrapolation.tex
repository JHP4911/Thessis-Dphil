...
\end{equation}
Where $w_{obs} = 1/\sigma_{obs}^2$ is the weighting term and $\sigma_{obs}$ is the standard deviation of the observation, $I_{obs}(D_{obs})$ is the observed intensity value at dose $D_{obs}$ and $I_{calc}$ is the calculated intensity using equation \ref{eq:Extrapolation regression function}.
...
An additional procedure to prevent overfitting was to ensure that the fit correlated with the data.
...
A bad correlation suggests that there were problems with the fit as shown in Figure~\ref{fig:Bad correlation bad fit - Extrapolation method} where the correlation coefficient was 0.209.
However a bad correlation value did not always suggest a bad fit as shown in Figure~\ref{fig:Bad correlation good fit - Extrapolation method}, where the correlation was low (0.231) but the fit looks reasonably good when accounting for the noise level.
\begin{figure}
        \centering
        ...
        The bad correlation coefficient in this case is clearly due to the noisy measurements but the fit looks reasonable given that the intensity predictions are consistently within 1 standard deviation of every observation.}
...
\begin{enumerate}
    \item the first $M$ observations are significantly above zero, where $M$ is a user defined integer value. This value was set to the same value as the minimum number of observations required to perform a regression fit.
...
\textcolor{red}{
    \begin{myenumerate}
        \item \hypertarget{todo:change figure}{\textbf{TODO:}Figure~\ref{fig:Data overfitting to small values - Extrapolation method} needs to change.}
        It doesn't completely fit with what is said.
    \end{myenumerate}
}
...
\begin{enumerate}
    \item in the \cite{diederichs2003} the standard deviation is inflated by the addition of a term which involves the standard deviation of a decay factor which is assumed common to all observations in a dataset.
...
The $R_{fit}$ metric was created for this purpose and is given by
\begin{equation}
...
$R^{hkl}_{err}$ is of a very similar form to a weighted relative error which is performed over all observations of a reflection.
...
In this case, the $model \equiv J$, where $J$ is the expected zero-dose intensity, and $data \equiv I, D$ where $I$ represents the observed intensity and $D$ the observed dose.
...
\begin{figure}
  \centering
    ...
    \caption{Logarithm of the mean intensities in 14 resolution shells for the zero-dose dataset and theoretical mean intensity values at dose $D_1$.
...
A smoothing spline was used to interpolate scale factors in the present work Figure~\ref{fig:Scale factors, smoothing interpolation - Extrapolation method} .
\begin{figure}
        \centering
        \begin{subfigure}[b]{1\textwidth}
                \centering
...
\end{equation}
where $f$, is the amount of $s(D)$ that actually multiplies $J$ and $A$ and $B$ are user specified parameters that determine where the inflection point occurs and the steepness of the function slope respectively.
...
\end{equation}
Finding the exact value of the marginal distribution of the data (the denominator in equation \ref{eq:Posterior for probabilistic extrapolation, with marginal denominator - Extrapolation method}) is not practical and is an active area of research.
